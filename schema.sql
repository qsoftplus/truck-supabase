...
create table public.loads (
  ...
  note text,
  pay_term text default 'To Pay', -- 'To Pay', 'Advance'
...
create table public.expenses (
  id uuid default uuid_generate_v4() primary key,
  trip_id uuid references public.trips(id) on delete cascade not null,
  category text not null, -- 'AdBlue', 'Salary', 'Loading', 'Unloading', 'RTO', 'Fastag', 'Billing', 'Other'
  title text, -- For Billing/Other specific names
  amount numeric default 0,
  liters numeric, -- For AdBlue/Oil
  percentage numeric, -- For Salary
  created_at timestamp with time zone default timezone('utc'::text, now()) not null
);
...
-- RLS Policies (Optional but recommended - simplifying for initial setup to allow public access/anon key interaction if needed, ideally restrict)
